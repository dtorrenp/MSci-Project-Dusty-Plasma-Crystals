...
>>>>>>> 2d20516aee6243b65cd5b6aec1f06dda1e7cb098
...
>>>>>>> 2d20516aee6243b65cd5b6aec1f06dda1e7cb098
    ...
    std::vector<double> f_der(std::vector<double> W_vec_f){
        ...
        //std::cout << rad_force_mag*(W_vec_f[0]/radial) << "," <<  - (alpha_n*W_vec_f[3])/mass << "," << + a_c[0] << "," <<  + (therm_coeff*dist(generator))/mass << std::endl;
        f.push_back(rad_acc_mag*(W_vec_f[0]/radial) - (alpha_n*W_vec_f[3])/mass -  M_PI*pow(grain_R,2)*m_i*n_i0*pow(W_vec_f[3],2)/mass + a_c[0] + (therm_coeff*dist(generator))/mass + (therm_coeff_i*dist(generator))/mass);//drag, sheathe and coloumb force and ion drag force
        f.push_back(rad_acc_mag*(W_vec_f[1]/radial) - (alpha_n*W_vec_f[4])/mass - M_PI*pow(grain_R,2)*m_i*n_i0*pow(W_vec_f[4],2)/mass  + a_c[1] + (therm_coeff*dist(generator))/mass + (therm_coeff_i*dist(generator))/mass);
        //z component
        if (W_vec_f[2] > z_se){
