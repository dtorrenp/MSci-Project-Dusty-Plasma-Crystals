...
const double temp_time_limit_thermalize = 16;//time run with the new temperture is this time minus the time limit                                                                                 v
...
class Dust_grain{
    ...
    std::vector<double> f_der(std::vector<double> W_vec_f, std::vector<double> brownian){
        ...
        if (W_vec_f[2] < z_se){
            ...
            f.push_back(charge*Sheath_E/m_D - g_z  + a_c[2] - (alpha_n*W_vec_f[5])/m_D  + (therm_coeff*brownian[2])/m_D);//drag, sheathe, gravity, coloumb force and ion drag force;
        }
        else{
...
class Load_Dust_Container{
    ...
    void update_neutral_temp_laser(double T_n_EV){
        ...
        std::cout << T_n_EV << "," << T_n_EV_init << std::endl;

        for(int i = 0; i <  Dust_grain_list.size(); i++){
            std::vector<double> pos (Dust_grain_list[i].W_vec.begin(),Dust_grain_list[i].W_vec.begin() + 3);
            if ((std::abs(pos[0]) < lambda_D) && (23*lambda_D < pos[2] < 26*lambda_D)){
                Dust_grain_list[i].update_temp(T_n_EV);
            }
        }
    }
...
int main(){
    ...
    const std::string load_file = "HPC_Data/Final_Periodic_Dust_grain_max_100_Final_Temperature_462.033326_frames_500001.csv";
    //read in the data
    ...
    std::string filename = "HPC_Data/Loaded_Periodic_Dust_grain_max_" + std::to_string(Dusty_plasma_crystal.dust_grain_max);
    ...
    std::string filename_end = "HPC_Data/Loaded_Final_Periodic_Dust_grain_max_" + std::to_string(Dusty_plasma_crystal.dust_grain_max);
